...
@Override
	...
	private boolean isValid(HttpServletRequest request, List<String> messages) {
		...
		if (50 < title.length()) {
			messages.add("タイトルは50字以下で入力してください");
		}
		if (StringUtils.isBlank(text) ==true) {
			messages.add("投稿を入力してください");
		}
		if (201 < text.length()) {
			messages.add("投稿は1000字以下で入力してください");
		}
		if (((StringUtils.isBlank(category))&&categorybox.matches("") )==true) {
			messages.add("カテゴリを入力してください");
		}
		if (10 < category.length()) {
			messages.add("カテゴリは10字以下で入力してください");
		}
